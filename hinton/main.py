...
def train(model, data_loader, optimizer, epoch, writer):
    ...
    for batch_idx, (data, target) in enumerate(tqdm(data_loader, unit='batch')):
        ...
        writer.add_scalar('train/total_loss', loss.data, global_step)
        writer.add_scalar('train/margin_loss', margin_loss.data, global_step)
        if args.use_reconstruction_loss:
            writer.add_scalar('train/reconstruction_loss', recon_loss.data, global_step)
        ...
        if batch_idx % args.log_interval == 0:
            ...
            tqdm.write(template.format(
                ...
                loss.data,
                margin_loss.data,
                recon_loss.data if args.use_reconstruction_loss else 0,
                acc,
                epoch_avg_acc))
