...
train_dataset = CIFAR10(path, train=True, transform=T.Compose([T.RandomRotation(
...
def train(epoch):
    ...
    for i, (img_batch, target) in enumerate(train_loader):
        ...
        angle = torch.Tensor(np.random.uniform(0, 2 * np.pi, size=(1,))).float()
        img_batch_rot = T.functional.rotate(img_batch, angle, resample=2)

        f_out, a_out, pose_out_1, img_out = model(img_batch.to(device))
        angle_1 = torch.arctan_(.....) ........
