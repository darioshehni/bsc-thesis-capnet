...
batch_size = 32
path = osp.join(osp.dirname(osp.realpath(__file__)), '..', 'data', 'CIFAR10')
train_dataset = CIFAR10(path, train=True, transform=T.Compose([T.Resize((28,28)), T.RandomRotation(
    180, resample=Image.BILINEAR), T.ToTensor()]), download=True)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
test_dataset = CIFAR10(path, train=False, transform=T.ToTensor())
test_loader = DataLoader(test_dataset, batch_size=batch_size)
test_rot_dataset = CIFAR10(
    path,
    train=False,
    transform=T.Compose([T.Resize((28,28)), T.RandomRotation(180, resample=Image.BILINEAR),
                         T.ToTensor()]))
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = GroupConvLayer(3, 16, kernel_size=5, initial=True)
        self.pool1 = PoolingCapsuleLayer(3, 16, num_iterations=1, f_in_size=16)
        ...
        self.recon1 = Linear(64 + 20 + 10, 1024)
        self.recon2 = Linear(1024, 1024*2)
        self.recon3 = Linear(1024*2, 784*3)

    def forward(self, img_batch):
        img_batch = img_batch.squeeze(-1)
        batch_size, in_channels, height, width = img_batch.size()
